        ...
        <v-row v-if="ready">
            <v-col cols="12">
                <v-card class="pa-2 pt-6">
                    <v-form ref="form" @submit.prevent="validate(model.status)" v-model="valid" lazy-validation>
                    <v-container grid-list-xs>
                        <v-row row wrap class="px-2 pt-2" align="start">
                            <label :class="{'news-label-full': $vuetify.breakpoint.xs, 'news-label': $vuetify.breakpoint.smAndUp}" for="title">Main Title:</label>
                            <v-textarea outlined dense
                                :rows="1" no-resize auto-grow
                                placeholder="Main Title" v-model="model.title"
                                :readonly="!editable"
                        ...
                        <v-row row wrap class="px-2 pt-2" align="start">
                            ...
                            <v-col class="pa-0 ma-0" cols="12" lg="12">
                                <v-row class="pa-0 ma-0">
                                    <label :class="{'news-label-full': $vuetify.breakpoint.xs, 'news-label': $vuetify.breakpoint.smAndUp}" for="date">Date:</label>
                                    <v-menu
                                        v-model="date_menu"
                        ...
                        <v-row row wrap class="px-2 pt-2" align="start">
                            <label :class="{'news-label-full': $vuetify.breakpoint.xs, 'news-label': $vuetify.breakpoint.smAndUp}" for="title">Custom Url:</label>
                            <v-text-field outlined dense
                                :placeholder="''+(model.id || 'id')" v-model="model.custom_url"
                                :readonly="!editable"
                                :prefix="getPrefix()"
                                :append-icon="(model.custom_url || model.id) ? 'mdi-content-copy' : ''"
                        ...
                        <v-row row wrap class="px-2 pt-2" align="start">
                            <label :class="{'news-label-full': $vuetify.breakpoint.xs, 'news-label': $vuetify.breakpoint.smAndUp}" for="image_url">Main Picture:</label>
                            <!-- :style="{'background-image': (model.image_url || '') }"  -->
                            <div id="container-img-uploader" v-if="editable" :class="{'container-img-full': $vuetify.breakpoint.xs, 'container-img': $vuetify.breakpoint.smAndUp, 'red-border': !file_ok}">
                                <v-img v-if="model.image_url" class="main-img"
                                    width="100%"
                                    contain
                                    :src="model.image_url"
                                ></v-img>
                                <label for="img-uploader" class="label-img-uploader">
                                    <v-img class="overlay-img" contain
                            ...
                            <p class="notif-danger" :class="{'container-message-full': $vuetify.breakpoint.xs, 'container-message': $vuetify.breakpoint.smAndUp}" v-if="!model.image_url">
                                <i class="error--text" style="font-size:10pt;">*Main Picture is required!</i>
                            </p>
                            <p class="notif-danger" :class="{'container-message-full': $vuetify.breakpoint.xs, 'container-message': $vuetify.breakpoint.smAndUp}" v-if="model.main_file && model.main_file.size > 2000000">
                                <i class="error--text" style="font-size:10pt;">*Max Upload Size is 2MB, your selected image size is : {{returnFileSize(model.main_file.size)}}</i>
                            </p>

                            <p class="notif-success" :class="{'container-message-full': $vuetify.breakpoint.xs, 'container-message': $vuetify.breakpoint.smAndUp}" v-if="model.main_file && model.main_file.size <= 2000000">
                                <i class="success--text" style="font-size:10pt;">*Your selected image size is : {{returnFileSize(model.main_file.size)}}</i>
                            </p>
                        </v-row>
                        ...
                        <v-row row wrap class="px-2 pt-2" align="start">
                            <label :class="{'news-label-full': $vuetify.breakpoint.xs, 'news-label': $vuetify.breakpoint.smAndUp}" for="description">Caption:</label>
                            <v-textarea outlined 
                                :rows="10" auto-grow no-resize
                                placeholder="Caption" v-model="model.description"
                                :readonly="!editable"
                                :rules="not_null_rules"
                                dense
                            ></v-textarea>
                        ...
                        <v-row row wrap class="pt-2 px-0" align="start">
                            <label class="px-2" :class="{'library-label-full': $vuetify.breakpoint.smAndDown, 'library-label': $vuetify.breakpoint.mdAndUp}" for="image_urls">Other Pictures ({{ model.images.length }}):</label>
                            <div :class="{'container-img-full': $vuetify.breakpoint.smAndDown, 'container-img': $vuetify.breakpoint.mdAndUp}">
                                <v-container fluid class="ma-0 pa-0">
                                    <v-row row wrap align="start" class="ma-0 pa-0">
                                        <v-col cols="12" sm="6" lg="4" xl="3" class="mb-4" v-if="model.images.length < 18">
                                            <div id="library-img-uploader" v-if="editable">
                                                <label for="img-uploader2" class="label-img-uploader">
                                                    <v-img class="overlay-img" contain
                                                        ...
                                                        alt="Image Upload"
                                                    />
                                                    <b class="picker-label">Click to Input some Image</b>
                                        ...
                                        <v-col cols="12" sm="6" lg="4" xl="3" class="mb-4" v-for="(img, idx) in model.images" :key="'library-'+idx">
                                            <div class="library-img" v-if="editable" :class="{'red-border': (img.file && img.file.size > 2000000)}">
                                                <v-img :key="imgs[idx]" class="news-library-image" @click="preview(img)"
                                                ...
                                                <!-- <div class="remove-button" style="border: 5px solid green;"> -->
                                                    <v-tooltip left>
                                                        <template v-slot:activator="{ on }">
                                                            <v-icon size="30" @click="removeImageLibrary(img)" v-on="on" class="remove-button" color="primary-darken-2">mdi-close</v-icon>
                                                        </template>
                                            ...
                                            <p class="text-center">
                                                &nbsp;<span :class="{'error--text': img.file.size > 2000000, 'success--text': img.file.size <= 2000000}" v-if="img.file">{{img.file_name}} ({{img.file_size}})</span>
                                            </p>
                                        </v-col>
                                    </v-row>
                                </v-container>
                        ...
                        <v-card-actions class="mt-6 pa-6" v-if="!submitting">
                            ...
                            <v-tooltip top>
                                <template v-slot:activator="{ on }">
                                    <v-btn v-on="on" elevation-0 width="100" outlined color="primary" type="submit">{{ model.status == "Published" ? "Post" : "Save"}}</v-btn>
                                </template>
                                <span>{{ model.status == "Published" ? getTooltip(0) : getTooltip(1)}}</span>
                            </v-tooltip>
                            &nbsp;&nbsp;&nbsp;&nbsp;
                            <v-tooltip top>
                                <template v-slot:activator="{ on }">
                                    <v-btn v-on="on" elevation-0 width="100" color="primary" @click="validate( (model.status == 'Published' ? 'Draft' : 'Published') )">{{ model.status == "Published" ? "Save" : "Post"}}</v-btn>
                                </template>
                                <span>{{ model.status == "Published" ? getTooltip(1) : getTooltip(0)}}</span>
                            </v-tooltip>
                        </v-card-actions>
                        ...
                        <v-card-actions class="mt-6 pa-6" v-else>
                            ...
                            <v-tooltip top>
                                <template v-slot:activator="{ on }">
                                    <v-btn v-on="on" elevation-0 width="100" outlined color="primary">
                                        <v-progress-circular
                                            indeterminate
                                            color="primary"
                                        ></v-progress-circular>
...
.news-label-full, .news-label, .library-label, .library-label-full{
    font-weight: 400;
...
.container-img-full, .container-message, .container-message-full{
    width: 100%;
}
...
export default {
    ...
    computed: {
      computedDateFormattedDatefns () {
        return this.model.date ? format(parseISO(this.model.date), 'EEEE, MMMM do yyyy') : ''
      },
    },
    ...
    methods: {
        ...
        postData() {
            ...
            arr.forEach( a => {
                if( item[a] && ((typeof item[a] == "string" ) || (typeof item[a] == "number" )) ){
                    // this.devLog(a)
                    // this.devLog(item[a])
                    if(a != "image_url"){ //image url may contain base64 string if have file
                        ...
                        case "images":
                            ...
                            if(this.model.images && this.model.images.length > 0){
                                ...
                                this.model.images.forEach(function (img, idx) {
                                    ...
                                    formData.append('arr_images['+idx+'][image_url]', img.image_url);
                                    if(img.file){
                                        formData.append('arr_images['+idx+'][file]', img.file);
                                        formData.append('arr_images['+idx+'][file_name]', img.file_name);
                                        formData.append('arr_images['+idx+'][file_size]', img.file_size);
                                    }
                                })
